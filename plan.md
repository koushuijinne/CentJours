...
> **Author**: Julien
> **Version**: v0.38 — 2026-03-24
> **Status**: Draft — M0-M4 完成，GATE 2 通过，测试166个；事件池按 ADR-008 分级（major 16 / normal 35 / minor 7），`historical_note` 与行动结算日志闭环已接通，政策 / 战役 / 行军 / 强化忠诚结算已玩家化，角色显示名已统一为数据驱动中文短称，累计 24 条旧事件完成 QA 修订，并累计新增 27 条中盘 / 联军 / 小人物 / 指挥 / 政治事件；零阻塞自动循环已收口到 `docs/codex_autonomous_workflow.md`，不再依赖外部监督器，且 Codex 可按 ADR-008 直接修文案，但禁止使用 reframing 句式
...
### M4: 内容填充（W15-W19）🔶 进行中 90%

**目标**: 用LLM批量生成所有文本内容，填充100天的完整事件池

**交付物**:

- [x] 历史事件池（当前58条，按 ADR-008 分级）`src/data/events/historical.json` ← 已补齐中盘 / 联军 / 小人物 / 指挥 / 根特-布鲁塞尔政治协同批次，仍需扩充至100+ 条
...
- [x] **文本 QA 持续推进**：累计 24 条旧事件完成史实/文风修订，减少过度文学化并补足说明性；累计新增 27 条事件按新标准直接入库
- [ ] 政策描述 + 后果文本
