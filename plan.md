...
> **Author**: Julien
> **Version**: v0.37 — 2026-03-24
> **Status**: Draft — M0-M4 完成，GATE 2 通过，测试165个；事件池按 ADR-008 分级（major 16 / normal 35 / minor 7），`historical_note` 与行动结算日志闭环已接通，政策 / 战役 / 行军 / 强化忠诚结算已玩家化，角色显示名已统一为数据驱动中文短称，累计 24 条旧事件完成 QA 修订，并累计新增 27 条中盘 / 联军 / 小人物 / 指挥 / 政治事件；零阻塞自动循环已收口到 `docs/codex_autonomous_workflow.md`，不再依赖外部监督器，且 Codex 仅负责结构/实现/验证，不承担文案写作
