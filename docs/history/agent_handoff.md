...
- `前端自动回归仍不够宽`：存读档、设置、地图交互边界、战斗/接见失败恢复和行军主流程已纳入 `GdUnit4`，`EventBus` 噪音也已清掉，但更多核心玩法状态流（战斗结算、接见结算、多日连续行军）还没进回归
