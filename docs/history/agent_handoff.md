...
1. 把行动经济从“一天一张牌”改成“至少一次行军 + 2 到 3 次决策”
2. 把教程 / 事件改成弹窗，并提供日志回看入口
3. 给玩家提供结局 / 战略目标入口
4. 先把中文试玩阶段的玩家可见文本清成全中文
5. 压缩常驻大面板占比，重构成地图优先布局
6. 上述修复同时继续维持 Windows CI、Rust 集成/属性测试和 `GdUnit4` 回归

## 当前已知缺口

- `真人试玩核心问题已暴露`：当前一天只能打一张牌，回合内决策密度不足；教程/事件没有弹窗和回看；玩家不知道结局目标；地图被大面板压缩；中文试玩阶段仍残留英文 UI
...
- 若继续按 2026-03-29 真人试玩修复，先处理行动经济、弹窗教程/事件、结局入口、中文收口和地图优先布局，再继续扩内容量
