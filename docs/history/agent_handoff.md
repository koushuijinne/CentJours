...
- 行军预览现在会根据目标节点容量和当前库存给出轻量补给压力提示，至少能在确认前告诉玩家“这一步大致安全还是危险”。
...
1. 把补给系统继续产品化：补给来源、前线压力、玩家可控补给手段、预判反馈与失败解释
...
- `补给反馈还不够前置`：玩家能在结算后看到补给变化，但行军前预判、为何断补给、如何补救仍需继续产品化
- `补给预判还不是权威值`：当前行军前反馈是 UI 轻量估计，不是 Rust 权威预测；后续仍要把预测接口正式下放到引擎层
