...
| 测试 | Windows `cargo test 215/215` (state_tests.rs 拆分后仍通过) + GdUnit4 `26/26` + Windows CI + smoke |
...
- **i18n**: CSV 运行时加载 + 210 翻译键（zh/en），顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类/弹窗标题/战略概览/百科章节标题/日志框架/地图默认文本已 tr() 化，设置面板含语言切换；剩余叙事长文本和百科/教程/结局文案待翻译
