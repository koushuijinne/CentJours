...
- `前端自动回归仍不够宽`：存读档、设置、地图交互边界、战斗/接见失败恢复与成功推进、行军主流程和休整行动已纳入 `GdUnit4`，`EventBus` 噪音也已清掉，但多日连续行动、政策冷却和更多边界路径还没进回归
