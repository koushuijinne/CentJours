...
- Godot 前端自动回归已扩到 `GdUnit4 41/41`，当前 Windows 基线包含存读档槽位、槽位标签文案、覆盖确认、删除入口、覆盖/删除确认取消后返回槽位选择框、新局确认/取消、读档取消、存档槽位取消恢复、保存页删除、设置弹窗打开/取消/读取已保存值/应用持久化/恢复默认/取消不污染已保存配置、叙事面板、区域任务显示、战斗弹窗取消、战斗提交失败恢复、接见禁用态、接见取消恢复、接见提交失败恢复、结局弹窗显示/重开/天数截断、地图 hover/锁定详情同锚点与滚动护栏、锁定后忽略他处 hover、重复点击取消锁定、空白画布清空交互、右键缩放复位保持锁定、读档成功后清空地图锁定、Windows Godot 主项目无头和 Windows smoke scene；Windows CI workflow 与本地脚本入口也已写入仓库。
...
- `前端自动回归仍不够宽`：存读档槽位覆盖/删除、取消确认返回、设置入口与重置/取消边界、战斗/接见失败恢复已纳入 `GdUnit4`，`EventBus` 噪音也已清掉，但更多地图交互边界还没进回归
