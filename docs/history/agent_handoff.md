...
- Rust 规则层最近一次完整回归基线是 Windows `189/189`；自动工作流后续不再把 Linux / WSL `cargo test` 当成默认验证路径。
...
- `补给玩法还不够显式`：底层压力已经接通，也已经有三张补给政策、补给角色 / 枢纽可视化，以及后勤态势 / 阶段目标 / 补给窗口 / 第二跳风险提示，但仍缺更强的长线运营感
