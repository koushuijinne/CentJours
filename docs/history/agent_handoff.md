...
| 测试 | Windows `cargo test 215/215` + GdUnit4 `77/77` (0 flaky, 0 warnings) + Windows CI + smoke |
...
- **布局溢出修复**: 视口高度预算系统 + RootLayout clip_contents + 侧栏/托盘面板最小尺寸动态封顶 + GdUnit4 回归，防止 UI 溢出窗口边界
...
- **i18n**: CSV 运行时加载 + 211 翻译键（zh/en），顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类/弹窗标题/战略概览/百科章节标题/日志框架/地图默认文本已 tr() 化，设置面板含语言切换；剩余叙事长文本和百科/教程/结局文案待翻译
...
- **四线态势指导层**: 政治 / 战场 / 外交 / 区域运营四线的状态、风险和下一步建议，已同步进侧栏、战略目标弹窗、百科和日志回看
...
- **Codex harness**: 新增根 `AGENTS.md`、`tools/codex_doc_sync_guard.sh`、`tools/codex_round_check.sh`、`tools/codex_harness_status.sh`、`tools/codex_pick_next_task.py`、`tools/codex_round_summary.py`、`tools/codex_cycle.sh`、`tools/codex_validation_scope.py`、`tools/codex_light_guard.sh` 和可安装 `.githooks/pre-commit + pre-push`；doc-sync 与 light guard 已统一使用 `tools/codex_python.sh` 的 PYTHON / python3 / python / py 自动回退（跳过 Microsoft Store 占位 stub），Windows 上 `python3` 未安装时不再阻止 commit/push
...
- 补给、派系和结局之间的长期因果解释已接入顶栏/侧栏/地图副标题和四线态势第一版，但还没覆盖更多中盘事件与区域任务链
