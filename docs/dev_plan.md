...
- Rust 规则层在 2026-03-24 复核通过 `166/166` 测试；当前数据基线为 `15` 名角色、`41` 个地图节点、`58` 条分级历史事件。
...
- `events::pool` 本轮新增文案护栏测试，禁止历史事件再次出现“不是……而是……”“真正的问题是……”等 reframing 句式；`cargo test` 当前已提升到 `166` 项。
...
**166 单元测试全部通过** | **事件池**: 58 条（major 16 / normal 35 / minor 7）
