...
- Rust 规则层在 2026-03-24 复核通过 `165/165` 测试；当前数据基线为 `15` 名角色、`41` 个地图节点、`58` 条分级历史事件。
...
- Codex 工作边界已明确：**不负责文案**。后续若涉及 `narratives`、`historical_note`、结局文本或营销文案，Codex 只负责结构、触发、数据接入、验证与工程集成。
...
**165 单元测试全部通过** | **事件池**: 58 条（major 16 / normal 35 / minor 7）
...
| 历史事件池 | `events/pool.rs` + `EventTier` 枚举 | 37 |
