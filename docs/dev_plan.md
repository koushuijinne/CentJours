...
- Rust 规则层在 2026-03-23 复核通过 `145/145` 测试；当前数据基线为 `15` 名角色、`41` 个地图节点、`31` 条分级历史事件。
...
**145 单元测试全部通过** | **事件池**: 31 条（major 13 / normal 17 / minor 1）
...
- `内容量不足`：历史事件只有 `31` 条，距离 `ADR-008` 的 `100+` 目标差距明显，100 天长局重玩性不够。
- `文本 QA 未收口`：`docs/advice/claude_event_history.md` 指出的若干问题只完成了首批 8 条修订，剩余事件仍需继续统一史实与文风。
...
- `main_menu.gd` `543` 行，`map_controller.gd` `656` 行，控制器拆分后仍有继续收口空间
...
| 三系统状态机 | `engine/state.rs` | 37 |
| 历史事件池 | `events/pool.rs` + `EventTier` 枚举 | 25 |
...
| 优先级 | 项目 | 规模 | 决策理由 |
|--------|------|------|---------|
| **P0** | **历史事件扩充 31 → 100+，并完成文本 QA** | L | 这是百日长局成立的前提；没有足够事件量和文风/史实校正，Steam 首发会直接暴露内容重复与可信度问题。 |
| **P0** | **补齐政策后果文本与结局文本** | M | `historical_note` 展示链已接通，但政策与结局文本仍不完整，玩家在中后盘和结算阶段仍缺少足够解释。 |
