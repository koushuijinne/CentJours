...
- Rust 规则层在 2026-03-24 复核通过 `161/161` 测试；当前数据基线为 `15` 名角色、`41` 个地图节点、`55` 条分级历史事件。
...
**161 单元测试全部通过** | **事件池**: 55 条（major 16 / normal 33 / minor 6）
...
- `内容量仍不足`：历史事件已增至 `55` 条，但距离 `ADR-008` 的 `100+` 目标仍差 `45` 条，百日长局的重玩性还不够。
- `文本 QA 未收口`：首批 `8` 条旧事件修订和本轮新增 `6` 条终盘事件已落地，但全量 `55` 条仍未逐条统一史实、句式和注释风格。
...
| 将领关系网络 | `characters/network.rs` | 31 |
| 三系统状态机 | `engine/state.rs` | 41 |
| 历史事件池 | `events/pool.rs` + `EventTier` 枚举 | 34 |
...
| 优先级 | 项目 | 规模 | 决策理由 |
|--------|------|------|---------|
| **P0** | **历史事件扩充 55 → 100+，并完成文本 QA** | L | 这是百日长局成立的前提；没有足够事件量和文风/史实校正，Steam 首发会直接暴露内容重复与可信度问题。 |
