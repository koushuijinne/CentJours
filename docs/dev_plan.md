...
- Rust 规则层在 2026-03-23 复核通过 `150/150` 测试；当前数据基线为 `15` 名角色、`41` 个地图节点、`49` 条分级历史事件。
...
**150 单元测试全部通过** | **事件池**: 49 条（major 15 / normal 29 / minor 5）
...
- `政策/结局文本仍未完全收口`：本轮已把结局尾声与复盘提示真正接到 UI，行动后果标签也已读得懂，但政策执行日志仍偏技术化，结局仍只有单版本文本。
