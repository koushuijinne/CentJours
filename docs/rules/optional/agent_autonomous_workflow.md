...
- 自动循环默认不停。只有三种停止条件：核心玩法已经优化到 Steam 可上线级别并进入提审 / 发版收口、用户明确叫停、出现硬阻塞。
- “进入下一轮”适用于每一轮。不是第一轮结束后切一次，而是每轮结束后都必须立刻进入下一轮。
- 压缩摘要、阶段性汇报、提交 / 推送结果和一次回复都不是收尾。压缩摘要只用于防止上下文丢失，输出后必须继续。
- 达成某个阶段里程碑也不是停止条件。即使一组“三轮计划”已经完成，也只能把它当成切换下一组 `P0` 的依据，不能当成停机点。
...
10. 输出对话内压缩摘要并继续
   ...
   - 明确写出子 agent 是否已回收
   - 压缩摘要不是停机点；必须在摘要后立刻进入下一轮，直到核心玩法优化完成并达到 Steam 可上线级别
