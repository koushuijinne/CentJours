...
- Rust 规则层最近一次完整回归基线为 Windows `201/201`；自动工作流后续不再把 Linux / WSL `cargo test` 当成默认验证路径。
...
- 第四个玩家可控补给政策 `secure_regional_corridor / 巩固区域走廊` 已接入：它会同时保线并加固当前驻地，把脆弱中继线先稳成可持续走廊，已接入政策表、叙事池、预判、UI 元数据和测试护栏。
...
- 引擎状态现在还会给出“区域运营压力”：当前这片线路到底是承压、脆弱、稳固中还是可持续；侧栏、地图副标题、行军预判、政策预览和终局复盘都会复用这层反馈。
...
| 优先级 | 项目 | 规模 | 决策理由 |
|--------|------|------|----------|
| **P0** | **把补给系统继续产品化：补给来源、前线压力、玩家可控补给手段、失败解释与教学** | L | `agent_chat_history` 已明确指出后勤是当前最有价值的玩法增深方向；现在已有四张补给政策、Rust 权威预判、补给角色 / 枢纽可视化、后勤态势 / 阶段目标提示、补给窗口提示、第二跳推进风险预判、阶段运营目标、当日行动计划、三日后勤节奏、区域运营链路、区域运营压力、风险拆解和行动后补救建议，下一步该补的是更强的区域运营感、前 10 天教学串联和失败归因闭环。 |
...
- `补给玩法还没完全产品化`：当前已经有四层玩家杠杆、补给角色与枢纽可视化，以及后勤态势 / 阶段目标 / 当日行动计划 / 三日后勤节奏 / 区域运营链路 / 区域运营压力 / 补给窗口 / 第二跳风险提示，但长线运营感和区域节奏仍不够完整。
