...
- 行军预览现在会给出目标节点的轻量补给压力提示，玩家在确认前已经能看到“高压 / 可维持 / 有望回升”的前置信息。
...
| 优先级 | 项目 | 规模 | 决策理由 |
|--------|------|------|----------|
| **P0** | **把补给系统继续产品化：补给来源、前线压力、玩家可控补给手段、权威预判反馈与失败解释** | L | `agent_chat_history` 已明确指出后勤是当前最有价值的玩法增深方向；现在已有首张补给政策和轻量预判，但还缺 Rust 权威预测、更多补给杠杆和明确教学。 |
...
- `补给反馈还不够权威`：玩家现在能在行军前看到轻量预判，但它还不是 Rust 权威预测；更细的结果解释和教学提示也还不够完整。
