...
- Steam 首发前继续把项目定义为“100 天高压政治-军事危机模拟器”，不转向长线沙盒。
- 优先补核心循环厚度、历史逻辑层次、验证链和首发表面完整度。
- 大本营第一 / 第三人称互动场景记为低优先级扩展方向，不进入当前首发 P0。
...
- 教学、当日行动建议、阶段复盘、结局前独白，可以用拿破仑第一人称增强代入。
- 历史事件正文、`historical_note`、联军情报、议会与外交动态，不强行改成拿破仑第一人称，继续保留第三人称或档案体。
- 原则是“主观层服务代入，客观层保证史实”。同一条内容若同时需要两者，就拆成正文与史注两层。
- 第一人称文本的语气以命令、判断、回忆为主，不走泛抒情路线。
