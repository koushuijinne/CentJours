...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新两轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩、设置弹窗锁定语义拆分、百科内容扩写，以及“点击空白清空地图详情”的自动化护栏。
...
- 当前已新增 `GdUnit4` 教程弹窗宽度断言、设置弹窗语义断言、百科内容断言和地图空白点击清空断言，并通过 Windows `64/64` 回归；后续仍要继续保留 Windows 真机换行/遮挡验收。
...
| 测试覆盖 | 高 | Rust 215 + GdUnit4 64 + Windows CI |
...
| S1-9 | 设置弹窗状态语义与几何稳定 | P0 | S | 已修几何跳变与“正在结束今天”误提示，并补对应回归；后续继续补真机观感验收 |
| S1-10 | 游戏内百科 / 术语说明入口 | P0 | M | 第一版已落地；当前可解释红黑指数、合法性、系统影响和提高路径，后续继续补补给与结局关联 |
| S1-11 | Tray 锁定原因分层 | P0 | S | 已拆 modal / resolving / processing / game_over 四类锁定文案，避免设置弹窗复用结算提示 |
...
- 地图“点击空白即清除详情”的交互仍有偶发不稳定，说明 map interaction contract 还需要继续补边界断言。
- 设置弹窗打开后的布局稳定性还没有自动化护栏，目前仍主要依赖真机观察。
- 红黑指数、合法性与行动节奏的解释仍分散在文案里，没有统一的游戏内百科入口。
...
- `GdUnit4` 测试按职责拆分到五个文件（共 58 tests）：
