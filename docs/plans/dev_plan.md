...
- Save / Load 已进入 `v4` 兼容阶段；最近一次权威回归基线是 Windows `cargo test 215/215`、Windows `GdUnit4 58/58`、Windows Godot 主项目无头和 smoke scene。
...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新一轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩和场景英文占位清理。
...
- 当前已新增 `GdUnit4` 教程弹窗宽度断言，并通过 Windows `58/58` 回归；后续仍要继续保留 Windows 真机换行/遮挡验收。
...
- `GdUnit4` 测试按职责拆分到五个文件（共 57 tests）：
  - `main_menu_flow_test.gd` — 初始化、核心行动流、行军、教程弹窗宽度回归（14 tests）
...
- Godot 前端 `GdUnit4` `58/58` 回归，覆盖主菜单初始化、日内行动、结束今天、教程弹窗、日志回看、存读档、难度恢复、设置音频滑条、战斗/接见/休整、政策冷却、地图交互、教程弹窗宽度回归等核心路径。
