...
| 本地化 | 45% | i18n 框架 + 182 翻译键（zh/en），顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类已 tr() 化，设置面板含语言切换；剩余：叙事长文本、百科/教程内容、结局文案、地图节点名 |
...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S5-1 | 引入 i18n 框架 | P0 | L | **深度推进** — CSV 运行时加载 + 182 个翻译键（zh/en），覆盖顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类，设置面板含语言切换；剩余：终局复盘叙事、补给政策引导、百科/教程内容、OUTCOME_TEXT 结局文案 |
