...
- Save / Load 已进入 `v4` 兼容阶段；最近一次权威回归基线是 Windows `cargo test 215/215`、Windows `GdUnit4 69/69`、Windows Godot 主项目无头和 smoke scene。
...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新六轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩、设置弹窗锁定语义拆分、百科内容扩写、“点击空白清空地图详情”的自动化护栏、设置/百科弹窗被外部关闭后的 Tray 恢复链、行动面板的预算提示 / 确认按钮 / 禁用原因 / 分区语义第一版、“结局目标 / 百科 / 日志回看 / 终局复盘”四处共享的外交进度与路线说明，以及顶栏外交指标、侧栏战略焦点/主要风险/派系压力、地图副标题战略提示三处联动的中盘指导层。
...
- 当前已新增 `GdUnit4` 教程弹窗宽度断言、设置弹窗语义断言、百科内容断言、地图空白点击清空断言、“外部关闭弹窗后恢复交互”断言、顶栏外交/侧栏战略焦点/地图副标题断言，以及行动预算 / 按钮语义断言，并通过 Windows `69/69` 回归；后续仍要继续保留 Windows 真机换行/遮挡验收。
...
| S1-10 | 游戏内百科 / 术语说明入口 | P0 | M | 第二版已落地；当前可解释红黑指数、合法性、外交进度、系统影响与提高路径，并补了外部关闭后的交互恢复护栏；本轮又把同一套说明压进顶栏/侧栏/地图副标题，后续继续补补给与结局关联 |
...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S5-1 | 引入 i18n 框架 | P0 | L | **深度推进** — CSV 运行时加载 + 210 个翻译键（zh/en），覆盖顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类/战略概览/百科章节/日志/地图，设置面板含语言切换；剩余：终局复盘叙事、补给政策引导、百科/教程内容正文、OUTCOME_TEXT 结局文案 |
