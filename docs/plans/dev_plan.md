...
- `P0-9` 设置弹窗布局稳定性：打开设置后不能引发 UI 缩放或主界面几何跳变。
- `P0-10` 游戏内百科入口：解释红黑指数、合法性与核心系统含义，避免玩家靠猜。
...
| 测试覆盖 | 高 | Rust 215 + GdUnit4 58 + Windows CI |
...
| S1-7 | 地图交互清空与详情关闭链 | P0 | S | 解决点击空白区域偶发不能清空锁定详情的问题，并补可重复验证 |
| S1-8 | 行动面板禁用态与分区重排 | P0 | M | 行动点耗尽后政策统一灰掉；机动与政策继续拆开，减少“今天还能做什么”的误判 |
| S1-9 | 设置弹窗打开后的 UI 几何稳定 | P0 | S | 修复打开设置后 UI 比例或布局跳变，并补对应回归 |
| S1-10 | 游戏内百科 / 术语说明入口 | P0 | M | 给红黑指数、合法性、补给和核心节奏提供可随时打开的说明入口 |
