...
- Save / Load 已进入 `v3` 兼容阶段；最近一次权威回归基线是 Windows `211/211` Rust tests、Windows `GdUnit4 15/15`、Windows Godot 主项目无头和 smoke scene。
...
| **P0** | **继续扩 Godot `GdUnit4` 覆盖面** | M | 存读档槽位、槽位标签、新局确认、读档取消、战斗取消和接见禁用态已纳入回归，下一步继续补结局、设置和更多失败恢复边界。 |
...
- Godot 前端当前已有 `GdUnit4` `15/15` 回归，已覆盖主菜单初始化、执行行动、存读档槽位、槽位标签文案、新局确认/取消、读档取消、叙事面板、区域任务显示、战斗弹窗取消、接见禁用态和地图交互；另保留 `src/dev/engine_smoke_test_scene.tscn` smoke 入口。
