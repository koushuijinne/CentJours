...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S1-1 | 行动经济重构 | P0 | XL | 把每天固定一张牌改成“基础行军 + 多次决策”的节奏 |
| S1-2 | 教程 / 事件弹窗化 + 日志回看 | P0 | L | 当前侧栏提示不够显式，需改为弹窗 + 回看入口 |
| S1-3 | 结局目标入口 | P0 | M | 给玩家一个可点击的结局 / 战略目标说明入口 |
| S1-4 | 中文优先 UI 文本收口 | P0 | M | 当前中文试玩阶段，先清玩家可见英文残留 |
| S1-5 | 地图优先布局重构 | P1 | L | 对齐 P 社参考图，缩小常驻大面板占比 |
