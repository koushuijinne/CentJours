...
- 当前内容规模为 `15` 名角色、`41` 个地图节点、`100` 条历史事件；补给、政治、历史日志、存档读档和主菜单主循环都已接通。
- Save / Load 已进入 `v4` 兼容阶段；最近一次权威回归基线是 Windows `cargo test 215/215`、Windows `GdUnit4 77/77`、Windows Godot 主项目无头和 smoke scene。
...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新七轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩、设置弹窗锁定语义拆分、百科内容扩写、“点击空白清空地图详情”的自动化护栏、设置/百科弹窗被外部关闭后的 Tray 恢复链、行动面板的预算提示 / 确认按钮 / 禁用原因 / 分区语义第一版、“结局目标 / 百科 / 日志回看 / 终局复盘”四处共享的外交进度与路线说明，以及顶栏外交指标、侧栏战略焦点/主要风险/派系压力、地图副标题战略提示三处联动的中盘指导层；当前又把政治 / 战场 / 外交 / 区域运营四线态势、区域运营任务与下一步建议同步进侧栏、战略目标弹窗、百科和日志回看。
...
- `P0-12` 中盘四线指导层：把政治、战场、外交、区域运营四条线的当前状态、风险和下一步建议同步到主界面与弹窗入口。
...
- `06` 轮新增 2 条反馈（2026-04-15）：
  - 画面溢出窗口边界，布局架构需要整体防溢出机制 → **已修复**：视口高度预算 + clip_contents + 动态封顶 + GdUnit4 回归
  - 翻译优先级调低，当前最重要的是完整 MVP → 后续优先级已调整
- 最新反馈新增一条高优先级 UI 回归：教程弹窗正文被压成窄列，出现近似竖排显示。
- 这说明“教程 / 事件弹窗已接入”不等于“弹窗版式已稳定”，S1 还不能视为收口。
- 当前已新增 `GdUnit4` 教程弹窗宽度断言、设置弹窗语义断言、百科内容断言、地图空白点击清空断言、“外部关闭弹窗后恢复交互”断言、顶栏外交/侧栏战略焦点/地图副标题断言，以及行动预算 / 按钮语义断言，并通过 Windows `77/77` 回归；后续仍要继续保留 Windows 真机换行/遮挡验收。
...
| 历史事件内容 | 100% | 100/100+ 条，Day 0-100 全覆盖 |
...
| 本地化 | 50% | i18n 框架 + 210 翻译键（zh/en），顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类/战略概览/百科章节/日志/地图已 tr() 化，设置面板含语言切换；剩余：叙事长文本、百科/教程内容正文、结局文案、地图节点名 |
| Steam 集成 | 0% | 无 Steamworks SDK、无成就、无云存档 |
| 设置系统 | 75% | 窗口模式 + UI 缩放 + 音频滑条 + 难度选择 + 语言切换已有，缺按键绑定 |
| 地图视觉 | 0% | 数据完整(41 节点)，渲染为线框，无美术 |
| 测试覆盖 | 高 | Rust 215 + GdUnit4 77 + Windows CI |
| UI 主题 | 55% | 帝国新古典风格主题已覆盖 9 种控件类型，还缺自定义字体和最终 polish |
...
| S1-3 | 结局目标入口 | P0 | M | 第二版已落地；顶栏已有“结局”入口，当前会展示外交进度、路线优先级、达成要点、风险与下一步建议，并已把这套路线判断同步到侧栏和地图副标题 |
...
| S1-10 | 游戏内百科 / 术语说明入口 | P0 | M | 第二版已落地；当前可解释红黑指数、合法性、外交进度、系统影响与提高路径，并补了外部关闭后的交互恢复护栏；本轮又补了区域运营怎么读、战略大仓/区域整补点/前线消耗点说明和当前运营快照 |
| S1-11 | Tray 锁定原因分层 | P0 | S | 已拆 modal / resolving / processing / game_over 四类锁定文案，并补上 modal 被外部关闭时的恢复链 |
| S1-12 | 四线态势与区域运营指导层 | P0 | M | 已落第一版；政治 / 战场 / 外交 / 区域运营四条线的状态、风险与下一步建议，已同步到侧栏、战略目标弹窗、百科和日志回看 |
...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S2-1 | ~~历史事件扩到 100+ 条~~ | P3 | XL | **已完成** — 100 条事件，Day 0-100 全覆盖 |
...
| S2-4 | 中期张力补强 (Day 20-80) | P1 | L | **第一版已落地** — 新增 16 条中盘事件（政治危机/联军压力/派系叛变/补给危机），Rust 新增 legitimacy/faction/supply/victories 触发条件 |
...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S4-1 | 自定义 UI 主题 | P0 | L | **第一版已落地** — 帝国新古典调色板，Panel/Button/ProgressBar/PopupPanel/ScrollBar/Slider/Tooltip/LineEdit/Separator 全套样式 |
...
| ID | 任务 | 优先级 | 规模 | 说明 |
|----|------|--------|------|------|
| S5-1 | 引入 i18n 框架 | P2 | L | **深度推进** — CSV 运行时加载 + 210 个翻译键（zh/en），覆盖顶栏/托盘/存读档/终局/侧栏/卡片/政策效果/叙事分类/战略概览/百科章节/日志/地图，设置面板含语言切换；剩余：终局复盘叙事、补给政策引导、百科/教程内容正文、OUTCOME_TEXT 结局文案 |
| S5-2 | 英文翻译 | P3 | XL | 全量 UI + 58-100 条事件 + 教程 + 结局文本，约 3-5 万字；当前优先级低于 MVP 核心玩法和发布级验收 |
...
| S6-2 | Windows 发布构建链 | P0 | M | **第一版已落地** — export_presets.cfg + build_release.cmd + release DLL 构建验证通过；阻塞：需安装 Godot export templates |
...
- 红黑指数、合法性、外交进度、行动节奏与结局路线已有统一百科 / 目标入口，但补给系统、派系反馈与结局之间的因果解释还可以继续深化。
