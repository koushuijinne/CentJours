...
- Save / Load 已进入 `v4` 兼容阶段；最近一次权威回归基线是 Windows `cargo test 215/215`、Windows `GdUnit4 66/66`、Windows Godot 主项目无头和 smoke scene。
...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新三轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩、设置弹窗锁定语义拆分、百科内容扩写、“点击空白清空地图详情”的自动化护栏，以及设置/百科弹窗被外部关闭后的 Tray 恢复链。
...
- 当前已新增 `GdUnit4` 教程弹窗宽度断言、设置弹窗语义断言、百科内容断言、地图空白点击清空断言和“外部关闭弹窗后恢复交互”断言，并通过 Windows `66/66` 回归；后续仍要继续保留 Windows 真机换行/遮挡验收。
...
| 测试覆盖 | 高 | Rust 215 + GdUnit4 66 + Windows CI |
...
| S1-8 | 行动面板禁用态与分区重排 | P0 | M | 行动点耗尽后政策统一灰掉；机动与政策继续拆开，减少“今天还能做什么”的误判 |
...
- `GdUnit4` 测试按职责拆分到五个文件（共 66 tests）：
  - `main_menu_flow_test.gd` — 初始化、核心行动流、行军、教程弹窗宽度回归、百科外部关闭恢复（17 tests）
