...
- 本轮新增：日内行动节奏改为“1 次机动槽 + 2 次决策点 + 手动结束今天”，前 10 天教程/历史事件/结局目标入口已弹窗化并可回看，玩家可见主 UI 文本继续向中文收口，地图区域占比已抬高；最新五轮又补了教程弹窗固定宽度护栏、侧栏第二轮压缩、设置弹窗锁定语义拆分、百科内容扩写、“点击空白清空地图详情”的自动化护栏、设置/百科弹窗被外部关闭后的 Tray 恢复链、行动面板的预算提示 / 确认按钮 / 禁用原因 / 分区语义第一版，以及“结局目标 / 百科 / 日志回看 / 终局复盘”四处共享的外交进度与路线说明。
...
| S1-3 | 结局目标入口 | P0 | M | 第二版已落地；顶栏已有“结局”入口，当前会展示外交进度、路线优先级、达成要点、风险与下一步建议 |
...
| S1-10 | 游戏内百科 / 术语说明入口 | P0 | M | 第二版已落地；当前可解释红黑指数、合法性、外交进度、系统影响与提高路径，并补了外部关闭后的交互恢复护栏；后续继续补补给与结局关联 |
