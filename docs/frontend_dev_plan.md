...
F3  决策托盘交互        ████████████  95% ✅ Rest 卡 + 政策卡 + hover scale + 回合闭环 + 执行后冷却态
...
8. 卡片冷却态目前仅为本回合视觉标记，引擎未暴露政策冷却 API，P2。
...
| ③ | `src/ui/main_menu.gd` | ✅ 决策卡片执行后冷却态：`_mark_card_cooldown()` + 新回合自动 `_reset_card_cooldowns()` |
