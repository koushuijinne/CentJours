...
F2  HUD 数据展示        ████████████  90% ✅ 真实引擎状态 + RN 氛围叠加层
F3  决策托盘交互        ████████████  90% ✅ Rest 卡 + 政策卡 + hover scale + 回合闭环
F4  地图与侧栏占位      ██████████░░  80% ✅ 叙事滚动日志 + 全将领忠诚度 + Line2D 路线
F5  视觉统一与动效      █████░░░░░░░  35% 🔶 RN 氛围已接，hover 已修，细节仍可深化
```
...
5. 地图区仍是静态战略感占位，尚未读取 `map_nodes.json` 做数据驱动布局（P3，非阻塞）。
6. 派系支持度无趋势方向箭头（P3，非阻塞）。
