...
fn ratio_to_result(ratio: f64) -> BattleResult {
    ...
    else if ratio > 0.6 { BattleResult::MarginalDefeat }
    else                { BattleResult::DecisiveDefeat }
}
...
mod tests {
    ...
    fn make_army(troops: u32, morale: f64, fatigue: f64, skill: f64) -> ForceData {
        ForceData { troops, morale, fatigue, general_skill: skill, supply_ok: true }
    }
    ...
    fn 疲劳100时战斗力减半() {
        ...
        let fatigue_out = resolve_battle(&fatigued, &vs_strong, Terrain::Plains, &mut rng_seeded(1));
    ...
    #[test]
    fn 补给不足惩罚25percent() {
        let supplied     = ForceData { supply_ok: true,  ..make_army(10_000, 80.0, 0.0, 70.0) };
        let unsupplied   = ForceData { supply_ok: false, ..make_army(10_000, 80.0, 0.0, 70.0) };
    ...
    fn 滑铁卢场景_英军高地防守() {
        ...
        assert!(defeat_count > 800, "威灵顿高地防守应大幅获胜: 败={}/1000", defeat_count);
        // 随机因子确保不是100%必败（偶有平局）
